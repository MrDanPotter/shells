...
function init(opts) {
  ...
  if (!projectRoot) throw new Error('a target directory is required');
  if (projectRoot === PKG_ROOT) {
    throw new Error('refusing to scaffold shells into its own source tree');
  }

  // Always scaffold into a named directory (never the bare cwd) — create it if it
  // does not exist yet. An existing directory is augmented in place.
  if (!fs.existsSync(projectRoot) && !dryRun) {
    fs.mkdirSync(projectRoot, { recursive: true });
  }

  const steps = [];

  // 1. vendor the kit. The included UI ships by default; --no-ui drops it for a
  //    user bringing their own front end.
  const skip = opts.noUi ? new Set(manifest.ui) : new Set();
  const toCopy = manifest.kit.filter(rel => !skip.has(rel));
  ...
  return { projectRoot, vendor: VENDOR, dryRun, ui: !opts.noUi, steps };
}

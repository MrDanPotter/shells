...
function register({ root, name, key, stateDir, app, session } = {}) {
  ...
  const entry = {
    ...
    root,
    stateDir,
    // How to launch this project's own app in dev mode: { cmd, url }. Preserved across
    // re-registration when not supplied, so a plain `register` refresh keeps it.
    app: app || (existing && existing.app) || null,
    // How to start this project's Claude session: { cmd }. Defaults to `claude` at
    // launch time when unset; set it to a project's dev command for the full hub-aware
    // session. Preserved across a plain re-register.
    session: session || (existing && existing.session) || null,

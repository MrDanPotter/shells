...
//   1. the scaffolder vendors exactly `kit` into a host project's .shells/;
//   2. package.json "files" mirrors `kit`, so the npm tarball carries it;
...
//
// What is deliberately NOT in `kit`: the scaffolder's own entrypoint (bin/), the
// tests, and package.json/README/LICENSE — none of which a vendored install needs.
// The included web UI (`reference/`) IS in the kit and ships by default; `--no-ui`
// drops it via the `ui` subset below.
